...
def compute_and_plot_metrics(request_results: List[Dict[str, Any]], gpu_monitor: Any, out_prefix: str = ""):
    ...
    first_times = [r.get('first_token_time') for r in successful if r.get('first_token_time') is not None]
    last_times = [r.get('last_token_time') for r in successful if r.get('last_token_time') is not None]
    ...
    if last_times:
        if gp_times and len(gp_times) >= 2:
            base = gp_times[0]
            for i in range(len(gp_times) - 1):
                t0 = gp_times[i] - base
                t1 = gp_times[i+1] - base
